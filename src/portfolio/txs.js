...
const defaultTxs = [
  { symbol: "BTC", coin: "bitcoin", value: 0 },
  { symbol: "LTC", coin: "litecoin", value: 0 },
  { symbol: "ETH", coin: "ethereum", value: 0 }
];
...
export default function() {
  ...
  emitter.remove = function(i) {
    return store
      .get()
      .then(txs => txs.slice(0, i).concat(txs.slice(i + 1)))
      .then(txs => store.save(txs))
      .then(txs => {
        emitter.emit("change", txs);
        return txs;
  ...
  emitter.save = function(txs) {
    return store.save(txs.filter(tx => tx.coin && !tx.__typename)).then(txs => {
      emitter.emit("change", txs);
      return txs;
    });
  };
